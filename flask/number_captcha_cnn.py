...
def show_image(file_path, file_name):
    im = Image.open(file_path + file_name)  ##文件存在的路径
    im.show() 
...
class captcha:
    ...
    def __init__(self, data_location):
        ...
        with self.graph.as_default():
            self.x = tf.placeholder(tf.float32, [None, dataset.image_height * dataset.image_width])
            self.y = tf.placeholder(tf.float32, [None, dataset.captcha_length * dataset.captcha_set_length])
    ...
    def train_captcha_cnn(self):
        ...
        with tf.Session() as sess:
            ...
            for i in range(150000):
                if i % 10 == 0:
                    batch_x_test, batch_y_test = self.dataset.next_batch(50)
                    ...
                    batch_x_train, batch_y_train = self.dataset.next_batch(50)
    ...
    def recognize_number(self,model_path):
        with self.graph.as_default():
            ...
            with tf.Session() as sess:
                ...
                # batch_x, batch_y, file_path, files_name = self.dataset.next_batch(10, True)
                batch_x, batch_y = self.dataset.next_batch(8)
                y_ = sess.run(out, feed_dict={self.x: batch_x, self.keep_prob: 1})
                y__ = sess.run(tf.argmax(tf.reshape(y_, [-1, self.dataset.captcha_length, self.dataset.captcha_set_length]), 2))
                for i in range(8):
                    vector = np.zeros(self.dataset.captcha_length*self.dataset.captcha_set_length)
...
if __name__ == '__main__':
    # D:\\Code\\TF\\data\\test1\\
    #captcha = captcha('D:\\Code\\TF\\data\\train\\')
    #captcha.train_captcha_cnn()

    # captcha = captcha('D:\\Code\\TF\\data\\test1\\')
    captcha = captcha('./static/data/2Bo27JO/')
    # captcha.test_captcha_cnn()
    result = captcha.recognize_number('./model/number/')
    print('===================')
    print(result)
    print('===================')
    #D:\Code\TF\data\test1
